...
- 查看可用 provider 列表
- 用统一的前缀 / 后缀 / 凭证流程创建临时邮箱会话
- 拉取收件箱消息列表
- 通过顶部邮箱栏切换、管理本地缓存的邮箱记录
- 查看单封邮件的纯文本与 HTML 内容
- 通过 API 路由进入附件下载
...
- **单页工作区**：`src/components/tpmail/app-shell.tsx` 负责 provider 切换、建箱、域名预取、缓存邮箱下拉、记录管理弹窗、读信、附件入口和邮件预览。
- **域名预取后再建箱**：切换 provider 后，前端会先请求 `/api/providers/[providerId]/domains` 获取可用后缀。
- **紧凑地址参数栏**：左侧地址参数区目前只保留邮箱前缀、邮箱后缀、按 provider 类型自动切换的凭证字段，以及创建 / 随机生成按钮。
- **浏览器托管会话**：`public_address`、token 型会话以及用户自带 API key 的邮箱会优先保存在当前浏览器，而不是依赖服务端进程内 `Map`。
- **缓存邮箱记录**：浏览器会保留最近 8 个邮箱会话，顶部邮箱栏提供快速下拉，管理按钮会打开独立表格弹窗，支持排序、多选和删除。
- **用户自带 API key**：以 `inboxes` 为代表的 `api_key` provider 可以由访问者自己输入 API key，避免共用站长托管凭据。
- **邮箱栏刷新进度**：顶部邮箱栏把地址、复制、历史记录、管理入口和自动刷新倒计时合并到一处，并用 10 秒填充式进度表现自动刷新节奏。
- **自动收件箱刷新**：创建邮箱后，前端每 10 秒轮询一次消息列表，不是实时推送。
...
前端主工作区集中在 `src/components/tpmail/app-shell.tsx`。默认会优先选择 `duckmail`，前提是该 provider 已启用。用户在这个组件里完成 provider 选择、邮箱创建、域名后缀切换、顶部邮箱栏历史记录切换、记录管理弹窗操作、消息轮询、单封邮件打开和附件下载入口访问。
...
- 浏览器托管的邮箱快照、缓存邮箱记录、以及用户自带的 API key 只保存在当前浏览器，不会自动同步到其他设备或浏览器配置文件。
...
- 收件箱更新依赖 10 秒轮询，不包含实时推送、WebSocket 或浏览器通知机制。
...
- 顶部缓存邮箱下拉与管理弹窗都依赖浏览器本地记录；换设备、换浏览器或清空本地存储后需要重新创建或恢复会话。
- 浏览器本地记录管理当前提供切换、排序、多选和删除，但不提供跨浏览器同步或服务端持久化。
